...
class AudioDevicesWindow(ctk_e.CTkToplevel):
    def __init__(self, *args, title='Audio Devices', **kwargs):
        ...
        self.audio_devices = AudioDevices.get_all_devices()
        self.label = None
...
class AudioDevicesFrame(ctk.CTkScrollableFrame):
    def __init__(self, master, audio_devices: list[AudioDevice], **kwargs):
        ...
        for name, devices in devices_per_type.items():
            count = len(self.audio_device_frames)
            frame = DeviceFrame(self, title=name, devices=devices)
            frame.grid(
                row=1 + count, column=0, padx=10, pady=10, sticky="nsew")
            self.audio_device_frames.append(frame)
...
class DeviceFrame(ctk_e.CTkVisibilityGridFrame):
    def __init__(self, master, title: str, devices: list[AudioDevice],
        ...
        for flow, flow_devices in sliders_per_flow.items():
            ...
            frame.grid(row=1 + count, column=0, padx=0, pady=1, sticky="ew")
